...
def sign_jwt(login: str) -> Dict[str, str]:
  ...
  """
  payload = {"login": login, "expires": (time.time() + Settings().EXPIRE_TIME)}
  return token_response(jwt.encode(payload, Settings().SECRET_KEY, algorithm=Settings().TOKEN_ALGORITHM))
...
def decode_jwt(token: str) -> dict:
  ...
  Returns:
    dict: The decoded token if valid and not expired, otherwise an empty dictionary.
  """
  decoded_token = jwt.decode(token.encode(), Settings().SECRET_KEY, algorithms=Settings().TOKEN_ALGORITHM)
